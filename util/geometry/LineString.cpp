...
QJsonObject GWSLineString::toGeoJSON(QJsonObject properties) const{
...
GWSLineString* GWSLineString::createReversed() const{
    const geos::geom::LineString* l = dynamic_cast<const geos::geom::LineString*>( this->inner_geometry );
    return new GWSLineString( dynamic_cast<geos::geom::LineString*>( l->reverse() ) );
}
